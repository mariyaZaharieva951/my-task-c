...
const Form = () => {
  ...
  const handleChange = (e: any) => {
    ...
    return (
             <form className="mx-auto max-w-9xl mb-5" onSubmit={handleSubmit}>
                <div className="relative flex justify-center pb-10">
                    <div className="bg-no-repeat bg-left">
                    <input
                     className="w-[13rem] rounded-l py-4 text-xs pl-8" 
                     placeholder="Мобилен телефон"
                      onChange={handleChange}
                      value={phoneNumber}
                     pattern='[0-9]{10}'
                     title='Моля, въведете 10 цифрен телефонен номер'
                     ></input>
                    <svg className="absolute top-5 transform -translate-y-1.5 h-6 w-4 pl-1 pb-1 ml-2 fill-current text-gray-400" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
